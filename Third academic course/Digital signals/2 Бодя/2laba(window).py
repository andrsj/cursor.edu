...
x, y = [i/fd for i in range(N)], [np.hamming(N)[i] for i in range(N)]
garmoniks0, garmoniks1 = [vl*sin(2*pi*f0*t/fd)*y[t] for t in range(N)], [vl*(sin(2*pi*f0*t/fd) + 2*sin(2*pi*f1*t/fd) + 3*sin(2*pi*f2*t/fd))*y[t] for t in range(N)]
